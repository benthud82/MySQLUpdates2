...
$result1 = $aseriesconn_can->prepare("SELECT PDWHSE as WHSE, PBSHJD as JDATE, count(PDCOMP) as INVLINES FROM A.ARCPCORDTA.NOTWPT, A.ARCPCORDTA.NOTWPS WHERE PBCOMP = PDCOMP and PBWHSE = PDWHSE and PBWCS# = PDWCS# and PBWKNO = PDWKNO and PBBOX# = PDBOX# and PDLOC# <> '*MSDS' AND PDPCKL - ROUND(PDPCKL,0) = 0 and PDWHSE = 11 and PBSHJD >= 14000 GROUP BY PBSHJD, PDWHSE ORDER BY PBSHJD ASC, PDWHSE ASC");
$result1->execute();
...
do {
    ...
    $sql = "INSERT INTO slotting.11invlinesshipped ($columns) VALUES $values ON DUPLICATE KEY UPDATE INVLINES=values(INVLINES)";
