...
$sqldelete = "DELETE FROM caseopps WHERE VCWHSE = 12";
...
$dallasmvcresult = $aseriesconn->prepare("SELECT VCWHSE, VCITEM, VCLOC#, VCPKGU, VCGRD5, PCCPKU FROM A.ARCPCORDTA.NPFMVC, A.ARCPCORDTA.NPFCPC WHERE PCITEM = VCITEM and PCWHSE = 0 and VCWHSE = 12 and VCPKGU = 1 and VCFTIR in ('L02','L04','L06') and VCITEM not in (select LMITEM from A.ARCPCORDTA.NPFLSM where LMWHSE = 12 and (LMTIER in('L17','L18','L03','L15') or LMTIER like 'C%')) and VCITEM in $uniquelist");
