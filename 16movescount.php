...
$excl = $conn1->prepare("SELECT CONCAT(MVITEM,MVTPKG,MVFZNE,MVTZNE,MVTYPE,MVDATE,MVREQT) as EXCLKEY FROM 16moves WHERE MVDATE BETWEEN DATE_SUB(NOW(), INTERVAL 13 DAY) AND NOW()");
$excl->execute();
...
$result = $aseriesconn_can->prepare("SELECT MVTITM, MVTPKG, MVFZNE, MVTZNE, MVTYPE, date(substr(MVREQD,1,4) || '-' || substr(MVREQD,5,2) || '-' || substr(MVREQD,7,2)) as DATE,  MVREQT FROM A.ARCPCORDTA.NPFMVE WHERE (MVTPKG <> 0) and MVCNFQ<>0 and (MVDESC like 'COMPLETED%' or MVDESC like 'MAN%') and MVWHSE = 16 and ((CURRENT DATE) - date(substr(MVREQD,1,4) || '-' || substr(MVREQD,5,2) || '-' || substr(MVREQD,7,2))) <= 8 GROUP BY MVTITM, MVTPKG, MVFZNE, MVTZNE, MVTYPE, date(substr(MVREQD,1,4) || '-' || substr(MVREQD,5,2) || '-' || substr(MVREQD,7,2)), MVREQT");
$result->execute();
