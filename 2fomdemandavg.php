...
$sqldelete = "DELETE from fomaverage WHERE FOMAVGWHSE = 2";
...
$sqlitem = "SELECT DISTINCT concat(FOMWHSE,FOMITEM, FOMPKGU) FROM FOMRAW WHERE ISFOM = 'Y' and FOMWHSE = 2 ORDER BY FOMITEM";
$queryitem = $conn1->prepare($sqlitem);
$queryitem->execute();
$itemarray = $queryitem->fetchAll(pdo::FETCH_COLUMN);
...
$result1 = $conn1->prepare("SELECT concat(FOMWHSE, FOMITEM, FOMPKGU), FOMPQTY, FOMDATE FROM fomraw WHERE ISFOM = 'Y' and FOMWHSE = 2");
$result1->execute();
...
do {
    ...
    while ($counter <= $maxrange) { //split into 5,000 lines segments to insert into merge table
        ...
        $resultkeys = _arraykeysearch($resultarray, 0, $itemarray[$counter]);
        ...
        $avg = intval(array_sum($datearray) / count($datearray));
        $count = intval(count($datearray));
        $item = substr($itemarray[$counter], 1, 7);
        $pkgu = intval(substr($itemarray[$counter], 8));
        $whse = intval(substr($itemarray[$counter], 0, 1));
    ...
    $sql = "INSERT IGNORE INTO slotting.FOMAVERAGE ($columns) VALUES $values";
