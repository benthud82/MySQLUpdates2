...
//code to update fomaverage mysql table for whse 2
...
$sqldelete = "DELETE FROM nonfomaverage WHERE NONFOMAVGWHSE = 3";
...
$sqlexclude = "SELECT DISTINCT CONCAT(FOMWHSE,FOMITEM, FOMPKGU), COUNT(FOMITEM) FROM FOMRAW WHERE ISFOM = 'N' AND FOMWHSE = 3 AND FOMDATE BETWEEN DATE_SUB(NOW(), INTERVAL 30 DAY) AND NOW() GROUP BY CONCAT(FOMWHSE,FOMITEM, FOMPKGU) HAVING COUNT(FOMITEM) >= 7 ORDER BY FOMITEM";
...
//$sqlitem = "SELECT DISTINCT concat(FOMWHSE,FOMITEM, FOMPKGU) FROM FOMRAW WHERE ISFOM = 'N' and FOMWHSE = 2 ORDER BY FOMITEM";
...
//    }
    ...
    $result1 = $conn1->prepare("SELECT concat(FOMWHSE, FOMITEM, FOMPKGU), FOMPQTY, FOMDATE FROM fomraw WHERE ISFOM = 'N' and FOMWHSE = 3 and FOMITEM = '" . $item . "'");
    $result1->execute();
