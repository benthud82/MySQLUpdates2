...
//update the largecustbybillto mysql table
...
$sqldelete = "TRUNCATE TABLE largecustbybillto";
$querydelete = $conn1->prepare($sqldelete);
$querydelete->execute();

$largecust = $aseriesconn->prepare("SELECT DISTINCT BOAN8,BOALPH,BOMLNM,BOADD1,BOADD3,BOADD4,BOCTY1,BOADDS,BOADDZ,BOCTR FROM HSIPCORDTA.NOTBOL, HSIPCORDTA.NOTWPS WHERE PBAN8 = BOAN8 and PBSHAN = BOSHAN GROUP BY BOAN8 HAVING (sum(case when PBSHJD >= $current_month_start_yyddd then PBBXVS else 0 end) >= 5000 or sum(case when PBSHJD >= $rolling_12_start_yyddd then PBBXVS else 0 end) >= 60000)");
$largecust->execute();
