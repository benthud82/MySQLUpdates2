...
//if (isset($var_whse)) {
//    $sqldelete = "DELETE FROM slotting.optimalbay WHERE Whse = $var_whse";
//} else {
...
//Result set for PPC sorted by highest PPC for items currently in L04
    $ppc = $useconn->prepare("SELECT VCWHSE,
                                         ...
                                         VCNDEP, 
                                         PICK_QTY_MN, 
                                         PICK_QTY_MN/VCADBS as DAILYPICKS, 
                                         cast(substring(VCNGD5,1,2) as INTEGER) * cast(substring(VCNGD5,4,2) as INTEGER) * VCNDEP as NEWGRIDVOL, 
                                         ((PICK_QTY_MN/VCADBS) / (cast(substring(VCNGD5,1,2) as INTEGER) * cast(substring(VCNGD5,4,2) as INTEGER) * VCNDEP)) * 1000 as PPC_CALC 
                                  FROM $useschema.NPFMVC, $useschema.NPTSLD 
                                  WHERE WAREHOUSE = VCWHSE and 
                                        VCITEM = ITEM_NUMBER and 
                                        VCLOC# = CUR_LOCATION 
                                        and VCWHSE = $whse 
                                        and VCFTIR in('C03','C05','C06') 
                                        and VCADBS > 0 
                                        and VCNDEP > 0  
                                        and cast(substring(VCNGD5,4,2) as INTEGER)  > 0 
                                        and cast(substring(VCNGD5,1,2) as INTEGER)  > 0 
                                  ORDER BY (PICK_QTY_MN/VCADBS) / (cast(substring(VCNGD5,1,2) as INTEGER) * cast(substring(VCNGD5,4,2) as INTEGER) * VCNDEP) DESC");
    $ppc->execute();
