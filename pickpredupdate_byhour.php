...
$sqldelete4 = "TRUNCATE  slotting.hist_casevol_merge ";
...
do {
    ...
    $sql = "INSERT IGNORE INTO slotting.hist_casevol_merge ($columns) VALUES $values";
...
$asyshistory = $conn1->prepare("SELECT 
                                                                ...
                                                                cutoff_group
                                                            FROM
                                                                slotting.hist_casevol_merge
                                                                    LEFT JOIN
                                                                slotting.printcutoff ON PBWHSE = cutoff_DC
...
//set print hour to 7 if predicted print date is greater than red date
    ...
    $sql = "INSERT IGNORE INTO slotting.hist_casevol ($columns) VALUES $values ON DUPLICATE KEY UPDATE hist_printdate=VALUES(hist_printdate), hist_printhrmn=VALUES(hist_printhrmn), hist_printhr=VALUES(hist_printhr), hist_reldate=VALUES(hist_reldate), hist_relhrmin=VALUES(hist_relhrmin)";
    $query = $conn1->prepare($sql);
    $query->execute();
    $maxrange += 4000;
} while ($counter <= $rowcount);
